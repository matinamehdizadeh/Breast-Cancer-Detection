...
from self_supervised.pret import pretrain
from self_supervised.pret import ssl_loss
...
class Trainer_MPCS:
    ...
    def save_model(self):
        if self.loss_list[self.current_epoch] < self.lowest_loss:
            os.makedirs(f"/content/drive/MyDrive/matinaMehdizadeh{config.result_path+self.experiment_description}",
                        exist_ok=True)
            torch.save(
                self.current_model.state_dict(),
                f"/content/drive/MyDrive/matinaMehdizadeh{config.result_path+self.experiment_description}/epoch_{self.current_epoch}_loss_{self.loss_list[self.current_epoch]}.pth"
            )
            self.lowest_loss = self.loss_list[self.current_epoch]

    def save_checkpoint(self):
        os.makedirs(
            f"/content/drive/MyDrive/matinaMehdizadeh{config.result_path+self.experiment_description}/checkpoints",
            exist_ok=True)
        torch.save(
            self.current_model.state_dict(),
            f"/content/drive/MyDrive/matinaMehdizadeh{config.result_path+self.experiment_description}/checkpoints/epoch_{self.current_epoch}_loss_{self.loss_list[self.current_epoch]}.pth"
        )