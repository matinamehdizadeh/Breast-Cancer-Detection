...
from Randaugment.augmenters.color.hsbcoloraugmenter import HsbColorAugmenter
...
def hed(image, factor):
    factor = random.randint(0, 10)/100
    #print('image',image.shape)
    image=np.transpose(image,[2,0,1])
...
crop_augmentation = A.Compose([
        A.RandomResizedCrop(height=460,width=700, p=1)
        ])

blur_augmentation = A.Compose([
        A.GaussianBlur((3, 3), (0.1, 2.0), p=1.0)
        ])

flip_augmentation = A.Compose([
        A.Flip(p=1)
        ])
...
def crop(image, args):
  return crop_augmentation(image=image)['image']

def blur(image, args):
  return blur_augmentation(image=image)['image']

def flip(image, args):
  return flip_augmentation(image=image)['image']

def color(image, args):
    return color_augmentation(image=image)['image']
...
def _parse_policy_info(name, prob, level, replace_value, augmentation_hparams,magnitude):
  ...
  if name=='Hsv':
      ...
      func = NAME_TO_FUNC[name]
      args = level_to_arg(augmentation_hparams)[name](magnitude)
  elif (name=='Blur') or (name=='Crop') or (name=='Flip') or (name == 'Color'):
      func = NAME_TO_FUNC[name]
      args = [0]
  else:
      func = NAME_TO_FUNC[name]
