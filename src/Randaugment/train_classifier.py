...
def train_classifier(dataset_dir, output_dir, n_epochs, batch_size, augmenter, lr, network_tag,
                     ...
                     randaugment=True, rand_m=5,  rand_n=2,ra_type=None, v1_type=None, v2_type=None, t1_type=None, t2_type=None):
    """
...
def build_classifier_model(input_shape, n_classes, lr, network_tag):
    ...
    if network_tag == 'basic':
        # Define classifier
        l2_factor = 1e-6
        efficient = EfficientNet_Model(pretrained=False)
        # 2. Initialized and load SSL pretraiend model which include backbone, MLP, and head
        pretrained_model = EfficientNet_MLP()
        pretrained_model.load_state_dict(torch.load('/content/drive/MyDrive/matinaMehdizadeh/results_bc/selfAll/pretMag.pth', map_location='cuda:0'))
        # 3. Use backbone part of pretrained model
        efficient.model = pretrained_model.backbone
        

        rfc1 = nn.Linear(1408, 9216)

        alexnet = models.alexnet(pretrained=True).classifier
        
        final_fc2 = nn.Sequential(nn.Dropout(0.5), nn.Linear(1000, 1))
        sig = nn.Sigmoid()
